...
class FID:
    ...
    def _calculate_activation_statistics(self, dataloader, model, classifier):
        classifier.eval()
        model.eval()
        ...
        with torch.no_grad():
            for image, _ in dataloader:
                ...
                real_activations.append(classifier.get_activations(image).cpu().numpy())
                fake_activations.append(classifier.get_activations(fake_image).cpu().numpy())
    ...
    def __call__(self, dataloader, model, classifier):
        m1, s1, m2, s2 = self._calculate_activation_statistics(dataloader, model, classifier)
